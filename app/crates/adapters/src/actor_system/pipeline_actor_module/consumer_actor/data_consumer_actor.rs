...
impl DataConsumerActor {
    ...
    pub fn record_received(&mut self) {
        self.flow_telemetry.record_received();
        self.mark_processed();
    }
...
impl ConsumerActorBridge {
    pub fn start_new_consumer_actor_with_impl(
        ...
        let actor = DataConsumerActor::new(data_source, data_processor, usage_scope, usage_meter);
...
impl SendActionToActor for ConsumerActorBridge {
    ...
    async fn get_flow_telemetry(&self) -> GetFlowTelemetryMessageResult {
        self.addr.send(GetFlowTelemetryMessage).await.map_err(|e| {
            PipelineLifecycleError::InternalCommunication {
                reason: format!(
                    "Failed to send flow telemetry message to consumer actor: {}",
                    e
                ),
            }
        })?
