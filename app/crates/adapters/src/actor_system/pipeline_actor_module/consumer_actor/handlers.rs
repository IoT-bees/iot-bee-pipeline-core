...
use crate::actor_system::pipeline_actor_module::consumer_actor::{
    ...
    messages::{
        ConsumerActorAction, ConsumerActorActionMessage, ConsumerActorState, RecordReceivedMessage,
    },
};
            ...
            ConsumerActorAction::StartConsuming => {
                ...
                let handle = tokio::spawn(async move {
                    ...
                    while let Some(data) = rx.recv().await {
                        actor_addr.do_send(RecordReceivedMessage);
                        let bytes = data.value().len() as u64;
                        if let Err(error) = usage_meter
                            .record(usage_scope, UsageEvent::Received { bytes })
                            .await
                        {
                            LOGGER.warn(&format!("No se pudo registrar recepción de uso: {error}"));
                        }
                        match usage_meter.allow_processing(usage_scope).await {
                            Ok(true) => {}
                            Ok(false) => {
                                let _ = usage_meter
                                    .record(usage_scope, UsageEvent::QuotaBlocked)
                                    .await;
                                LOGGER.warn("Mensaje descartado: cuota mensual de uso agotada");
                                continue;
                            }
                            Err(error) => {
                                LOGGER.warn(&format!("No se pudo evaluar cuota de uso: {error}"));
                                continue;
                            }
                        }
                        ...
                        .map(|result, actor, ctx| {
                            match result {
                                ...
                                Err(e) => {
                                    ...
                                    actor.mark_error(format!("consumer: {}", e));
                                    LOGGER.error(&format!(
            ...
            ConsumerActorAction::ChannelDied => {
                match self.state() {
                    ConsumerActorState::Consuming => {
                        LOGGER.warn(
                            "ChannelDied received while consuming. Transitioning to Reconnecting after delay.",
                        );
                        self.task_handle.take();
                        self.set_state(ConsumerActorState::Reconnecting);
                        self.set_operation_state(ActorOperationStatus::Degraded);
                        self.mark_error("consumer: channel died, reconnecting");
                        ctx.notify_later(
                            ConsumerActorActionMessage::start_consuming(),
                            ACTOR_RECONNECT_DELAY,
                        );
...
impl Handler<GetLastProcessedAtMessage> for DataConsumerActor {
...
impl Handler<GetLastErrorMessage> for DataConsumerActor {
    type Result = GetLastErrorMessageResult;
    fn handle(&mut self, _msg: GetLastErrorMessage, _ctx: &mut Self::Context) -> Self::Result {
        Ok(self.last_error().map(|s| s.to_string()))
    }
}

impl Handler<RecordReceivedMessage> for DataConsumerActor {
    type Result = ();

    fn handle(&mut self, _msg: RecordReceivedMessage, _ctx: &mut Self::Context) -> Self::Result {
        self.record_received();
    }
}

impl Handler<GetFlowTelemetryMessage> for DataConsumerActor {
    type Result = GetFlowTelemetryMessageResult;

    fn handle(&mut self, _msg: GetFlowTelemetryMessage, _ctx: &mut Self::Context) -> Self::Result {
