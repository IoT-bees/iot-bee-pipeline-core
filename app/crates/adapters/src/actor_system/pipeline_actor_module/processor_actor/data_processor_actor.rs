...
use super::super::general_messages::{
    ...
    GetLastProcessedAtMessage, GetLastProcessedAtMessageResult, SendActorActionMessage,
    SendActorActionMessageResult,
};
use super::super::general_ports::SendActionToActor;
use domain::value_objects::lifecycle_values::{ActorOperationStatus, ReplicaFlowTelemetry};
...
impl DataProcessorActor {
    ...
    pub fn record_validated(&mut self) {
        self.flow_telemetry.record_validated();
        self.mark_processed();
    }
    pub fn record_rejected(&mut self) {
        self.flow_telemetry.record_rejected();
        self.mark_processed();
    }
...
impl ProcessorActorBridge {
    pub fn start_new_processor_actor_with_impl(
        ...
        let actor = DataProcessorActor::new(data_store, data_processor, usage_scope, usage_meter);
...
impl SendActionToActor for ProcessorActorBridge {
    ...
    async fn get_last_processed_at(&self) -> GetLastProcessedAtMessageResult {
        self.addr
            ...
            .map_err(|e| PipelineLifecycleError::InternalCommunication {
                reason: format!(
                    "Failed to send last_processed_at message to processor actor: {}",
                    e
                ),
            })?
    }
    ...
    async fn get_last_error(&self) -> GetLastErrorMessageResult {
        self.addr.send(GetLastErrorMessage).await.map_err(|e| {
            PipelineLifecycleError::InternalCommunication {
                reason: format!(
                    "Failed to send last_error message to processor actor: {}",
                    e
                ),
            }
        })?
    ...
    async fn get_flow_telemetry(&self) -> GetFlowTelemetryMessageResult {
        self.addr.send(GetFlowTelemetryMessage).await.map_err(|e| {
            PipelineLifecycleError::InternalCommunication {
                reason: format!(
                    "Failed to send flow telemetry message to processor actor: {}",
                    e
                ),
            }
        })?
