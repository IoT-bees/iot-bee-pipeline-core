...
impl DataStoreActor {
    ...
    pub fn record_delivered(&mut self) {
        self.flow_telemetry.record_delivered();
        self.mark_processed();
    }
...
impl StoreActorBridge {
    pub fn start_new_store_actor_with_impl(
        ...
        let actor = DataStoreActor::new(Arc::clone(&external_store), usage_scope, usage_meter);
...
impl SendActionToActor for StoreActorBridge {
    ...
    async fn get_flow_telemetry(&self) -> GetFlowTelemetryMessageResult {
        self.addr.send(GetFlowTelemetryMessage).await.map_err(|e| {
            PipelineLifecycleError::InternalCommunication {
                reason: format!(
                    "Failed to send flow telemetry message to store actor: {}",
                    e
                ),
            }
        })?
