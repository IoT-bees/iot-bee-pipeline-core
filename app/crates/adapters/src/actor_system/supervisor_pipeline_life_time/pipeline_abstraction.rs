...
use domain::value_objects::lifecycle_values::{PipelinepartsStatus, ReplicaFlowTelemetry};
...
impl PipelineAbstractionController {
    ...
    pub async fn status(&self) -> Result<PipelinepartsStatus, IoTBeeError> {
        ...
        let (last_processed_at, last_error, flow_telemetry) =
        ...
        Ok(
            PipelinepartsStatus::new(consumer_status, processor_status, store_status)
                ...
                .with_flow_telemetry(flow_telemetry),
        )
    }
    ...
    async fn collect_telemetry(
        ...
        processor: &Arc<dyn SendActionToActor + Send + Sync>,
        store: &Arc<dyn SendActionToActor + Send + Sync>,
    ) -> (Option<DateTime<Utc>>, Option<String>, ReplicaFlowTelemetry) {
        ...
        let consumer_telemetry = consumer.get_flow_telemetry().await.unwrap_or_default();
        let processor_telemetry = processor.get_flow_telemetry().await.unwrap_or_default();
        let store_telemetry = store.get_flow_telemetry().await.unwrap_or_default();
        let flow_telemetry =
            ReplicaFlowTelemetry::merge(consumer_telemetry, processor_telemetry, store_telemetry);

        (last_processed_at, last_error, flow_telemetry)
    }
}
