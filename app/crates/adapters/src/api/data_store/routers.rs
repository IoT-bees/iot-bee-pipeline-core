...
pub async fn test_data_store(
    ...
    let message = use_case.test_data_store(org_id, &data_id).await?;
...
pub async fn create_data_store(
    ...
    use_case
        .create_data_store(org_id, &data_store_input)
        .await?;
...
pub async fn get_data_store(
    ...
    let org_id = require_org_id(&req)?;
    let data_id: u32 = id.into_inner();
    let data_store: PipelineDataStoreOutputModel =
        use_case.get_data_store_by_id(org_id, &data_id).await?;
...
pub async fn list_data_stores(
    ...
    let org_id = require_org_id(&req)?;
    let data_stores: Vec<PipelineDataStoreOutputModel> = use_case.get_data_store(org_id).await?;
...
pub async fn update_configuration(
    ...
    let data_store_input = PipelineDataStoreInputModel::try_from(body.into_inner())?;
    use_case
        .update_data_store_configuration(org_id, &data_id, &data_store_input)
        .await?;
...
pub async fn delete_data_store(
    ...
    let org_id = require_org_id(&req)?;
    let data_id: u32 = id.into_inner();
    use_case.delete_data_store(org_id, &data_id).await?;
