...
impl ResponseError for ApiError {
    fn status_code(&self) -> StatusCode {
        match &self.0 {
            ...
            IoTBeeError::DataSourceError(inner) => match inner {
                DataSourceError::NotSupported { .. } => StatusCode::NOT_IMPLEMENTED,
            ...
            IoTBeeError::AuthError(inner) => match inner {
                ...
                AuthError::RegistrationDisabled | AuthError::Forbidden => StatusCode::FORBIDDEN,
            ...
            IoTBeeError::SystemError(inner) => match inner {
                domain::error::SystemError::InvalidContactSettings { .. } => {
                    StatusCode::BAD_REQUEST
                }
                domain::error::SystemError::ProbeFailed { .. }
                | domain::error::SystemError::Persistence { .. } => {
                    StatusCode::INTERNAL_SERVER_ERROR
                }
            },
            ...
            IoTBeeError::NotificationError(inner) => match inner {
                domain::error::NotificationError::InvalidEmail { .. } => StatusCode::BAD_REQUEST,
                domain::error::NotificationError::Transport { .. } => StatusCode::BAD_GATEWAY,
            },
        }
    }
    ...
    fn error_response(&self) -> HttpResponse {
        match &self.0 {
            ...
            IoTBeeError::DataSourceError(e) => match e {
                DataSourceError::NotSupported { .. } => {
                    HttpResponse::NotImplemented().json(ErrorResponse {
                        error: e.to_string(),
                    })
                }
                _ => HttpResponse::build(StatusCode::BAD_REQUEST).json(ErrorResponse {
            ...
            IoTBeeError::NotificationError(e) => {
                HttpResponse::build(self.status_code()).json(ErrorResponse {
                    error: e.to_string(),
                })
            }
