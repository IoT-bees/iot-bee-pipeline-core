...
use actix_web::get;
use actix_web::{HttpRequest, HttpResponse, post, put, web};
...
pub async fn start_new_pipeline(
    ...
    use_case
        .start_new_pipeline(require_org_id(&req)?, pipeline_id)
        .await
        .map_err(|e| {
...
pub async fn stop_pipeline(
    ...
    use_case
        .stop_pipeline(require_org_id(&req)?, pipeline_id)
        .await
        .map_err(|e| {
...
pub async fn get_pipeline_status(
    ...
    let status = use_case
        .get_pipeline_status(require_org_id(&req)?, pipeline_id)
        .await
        .map_err(|e| {
...
pub async fn get_all_pipeline_status(
    ...
    let statuses = use_case
        .get_all_pipeline_status(require_org_id(&req)?)
        .await
        .map_err(|e| {
...
pub async fn update_pipeline_replication_factor(
    ...
    use_case
        .update_pipeline_replication_factor(require_org_id(&req)?, pipeline_id, replication_factor)
        .await
        .map_err(|e| {
            LOGGER.error(&format!(
