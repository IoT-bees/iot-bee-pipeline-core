...
fn body_webhook_valido() -> serde_json::Value {
    json!({
        "name": "erp-handoff",
        "dataStoreConfiguration": {
            "persistenceType": "WEBHOOK",
            "url": "https://client.example.com/api/iot/records",
            "bearer_token": "client-token"
        },
        "dataStoreDescription": "Entrega de registros validados al ERP del cliente"
    })
}
...
#[actix_web::test]
async fn post_data_store_webhook_valido_devuelve_201() {
    let use_case: Arc<dyn DataStoreUseCases + Send + Sync> = Arc::new(UseCaseVacio);
    let use_case_data = web::Data::from(use_case);
