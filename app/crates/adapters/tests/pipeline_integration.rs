...
use domain::usage::entities::{UsageEvent, UsageScope};
...
struct NoopUsageMeter;

#[async_trait]
impl UsageMeter for NoopUsageMeter {
    async fn allow_processing(&self, _: UsageScope) -> Result<bool, IoTBeeError> {
        Ok(true)
    }

    async fn record(&self, _: UsageScope, _: UsageEvent) -> Result<(), IoTBeeError> {
        Ok(())
    }
}
...
fn montar_pipeline(
    ...
    let usage_scope = UsageScope {
        organization_id: 1,
        pipeline_id: 1,
    };
    let usage_meter: Arc<dyn UsageMeter> = Arc::new(NoopUsageMeter);

    let store_bridge =
        StoreActorBridge::start_new_store_actor_with_impl(spy, usage_scope, usage_meter.clone());
    ...
    // ));
    let processor_bridge = ProcessorActorBridge::start_new_processor_actor_with_impl(
        store_bridge,
        data_process,
        usage_scope,
        usage_meter.clone(),
    ...
    let _consumer_addr = ConsumerActorBridge::start_new_consumer_actor_with_impl(
        source,
        processor_bridge,
        usage_scope,
        usage_meter,
...
async fn pipeline_propaga_todos_los_mensajes_de_extremo_a_extremo() {
    ...
    // HashMap serialization order is non-deterministic; compare parsed values instead of strings.
    let recibidos_lecturas: std::collections::HashSet<i64> = datos
        .iter()
        .filter_map(|s| serde_json::from_str::<serde_json::Value>(s).ok())
        .filter_map(|v| v.get("lectura").and_then(|x| x.as_i64()))
        .collect();
    for i in 0..N as i64 {
        assert!(
            recibidos_lecturas.contains(&i),
            "Falta lectura={i} en los payloads recibidos: {recibidos_lecturas:?}"
        );
    }
}
