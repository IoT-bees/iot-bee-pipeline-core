...
pub trait LicenseUseCases {
    ...
    async fn retry_billing_event(
        &self,
        organization_id: i64,
        id: i64,
    ) -> Result<BillingEvent, IoTBeeError>;
}
    ...
    T: LicenseRepository + Send + Sync,
{
    async fn status(
        ...
        let subscription = self.repository.get_subscription(organization_id).await?;
    ...
    async fn activate(
        ...
        self.repository
            .upsert_subscription(organization_id, &subscription)
            .await?;
        let limits = effective_limits(
            self.repository.as_ref(),
            self.plan_repository.as_ref(),
    ...
    async fn deactivate(
        ...
        self.repository
            .deactivate_subscription(organization_id)
            .await?;
        let limits = effective_limits(
            self.repository.as_ref(),
            self.plan_repository.as_ref(),
    ...
    async fn sync_stripe_subscription(
        ...
        let subscription = self
            .repository
            .sync_stripe_subscription(organization_id, sync)
            .await?;
        ...
        if sync.stripe_payment_status.as_deref() == Some("failed") {
            ...
            tokio::spawn(async move {
                match users.find_admin_by_org_id(organization_id).await {
                    Ok(Some(admin)) => {
    ...
    async fn list_billing_events(
        ...
        self.repository
            .list_billing_events(organization_id, limit)
            .await
    }
    ...
    async fn list_billing_events_page(
        ...
        self.repository
            .list_billing_events_page(organization_id, cursor, limit)
            .await
    }
    ...
    async fn retry_billing_event(
        ...
        let event = self
            .repository
            .get_billing_event(organization_id, id)
            .await?
        ...
        match parse_billing_event_payload(&event.payload) {
                ...
                .await
            {
                Ok(_) => {
                    self.repository
                        .mark_billing_event_processed(organization_id, id, true, None)
                        .await?;
                }
                Err(e) => {
                    let msg = e.to_string();
                    self.repository
                        .mark_billing_event_processed(organization_id, id, false, Some(&msg))
                        .await?;
                    return Err(e);
                }
            },
...
{
    let subscription = license_repo.get_subscription(organization_id).await?;
    let now = Utc::now();
