...
pub fn usage_quota(to: &str, consumed_messages: u64, included_messages: u64) -> EmailMessage {
    EmailMessage {
        to: to.to_string(),
        subject: "uso mensual de mensajes — iot bees".into(),
        html: format!(
            "<p>Tu organización ha usado {consumed_messages} de {included_messages} mensajes entregados este mes.</p>\
             <p>Revisa la facturación antes de agotar la cuota para evitar que se pausen nuevos procesamientos.</p>"
        ),
        text: format!(
            "Uso mensual: {consumed_messages} de {included_messages} mensajes entregados. Revisa la facturación antes de agotar la cuota."
        ),
    }
}
