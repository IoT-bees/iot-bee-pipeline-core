...
pub trait PipelineLifecycleCases {
    ...
    async fn start_new_pipeline(&self, organization_id: i64, id: u32) -> Result<(), IoTBeeError>;
    async fn stop_pipeline(&self, organization_id: i64, id: u32) -> Result<(), IoTBeeError>;
    async fn get_pipeline_status(
        &self,
        organization_id: i64,
        id: u32,
    ) -> Result<PipelineStatusReport, IoTBeeError>;
    async fn get_all_pipeline_status(
        &self,
        organization_id: i64,
    ) -> Result<Vec<PipelineStatusReport>, IoTBeeError>;
    /// Vista de observabilidad global sin acceso a configuraciones ni recursos.
    async fn get_all_pipeline_status_for_system(
        &self,
    ) -> Result<Vec<PipelineStatusReport>, IoTBeeError>;
...
impl PipelineLifecycleCases for PipelineLifecycleCasesImpl {
    async fn start_all_pipelines_in_system(&self) -> Result<(), IoTBeeError> {
        ...
        for pipeline in pipelines {
            ...
            let Some(organization_id) = self
                .pipeline_controller
                .organization_id_for_pipeline(&pipeline.id())
                .await?
            else {
                LOGGER.error(&format!("[pipeline id={}] organization not found", id));
                failed += 1;
                continue;
            };

            let limits = match effective_limits(
            ...
            .await
            {
                Ok(value) => value.limits,
                Err(error) => {
                    LOGGER.error(&format!(
                        "[pipeline id={}] failed to resolve plan limits: {}",
                        id, error
                    ));
                    failed += 1;
                    continue;
            ...
            let restricted = self
                ...
                .await?
                .is_some_and(|subscription| subscription.is_restricted());
            let pipeline_count = self
                .pipeline_controller
                .count_pipelines(organization_id)
                .await?;
            if restricted
                || pipeline_count > limits.max_pipelines
                || pipeline.pipeline_replication() > limits.max_replicas_per_pipeline
            {
                LOGGER.warn(&format!(
                    "[pipeline id={}] skipped — exceeds current organization plan limits",
                    id
                ));
                skipped += 1;
                continue;
    ...
    async fn start_new_pipeline(&self, organization_id: i64, id: u32) -> Result<(), IoTBeeError> {
        ...
        let pipeline = self
            .pipeline_controller
            .get_pipeline_by_id(organization_id, &pipeline_id)
            .await?
            .ok_or_else(|| {
            ...
            .await?
        {
            if subscription.is_restricted() {
                return Err(domain::error::LicenseError::LimitExceeded {
                    reason: "your last payment failed; update your card before starting pipelines"
                        .into(),
                }
                .into());
            }
            ...
            > limits.max_pipelines
        {
            return Err(domain::error::LicenseError::LimitExceeded {
                reason: format!(
                    "your current plan allows {} pipelines; remove or upgrade before starting more",
                    limits.max_pipelines
                ),
            }
            .into());
        ...
        let data_store = self
            .data_store_repository
            .get_pipeline_data_store_by_id(organization_id, &DataStoreId::new(pipeline.store_id())?)
            .await?
            .ok_or_else(|| {
                LOGGER.error(&format!(
        ...
        let update_pipeline_state = self
            .pipeline_controller
            .update_pipeline_state(organization_id, pipeline.id(), true)
            .await;
    ...
    async fn stop_pipeline(&self, organization_id: i64, id: u32) -> Result<(), IoTBeeError> {
        ...
        self.pipeline_controller
            .update_pipeline_state(organization_id, &pipeline_id, false)
            .await?;
        self.pipeline_lifecycle.stop(&pipeline_id).await
    }
    ...
    async fn get_all_pipeline_status(
        ...
        for report in reports {
            ...
            let pipeline = self
                .pipeline_controller
                .get_pipeline_by_id(organization_id, &DataStoreId::new(pid)?)
                .await
                .ok()
                .flatten();
    ...
    async fn get_all_pipeline_status_for_system(
        ...
        self.pipeline_lifecycle.get_all_status().await
    }
    ...
    async fn update_pipeline_replication_factor(
        ...
        self.pipeline_controller
            .update_pipeline_replication_factor(organization_id, &pipeline_id, &replication_factor)
            .await
    }
}
