...
pub enum DataSourceError {
    ...
    #[error("data source type '{kind}' is not yet supported")]
...
pub enum DomainValidationError {
    ...
    #[error("AST depth exceeds maximum allowed limit of {limit}")]
    AstTooDeep { limit: usize },
...
pub enum SystemError {
    ...
    #[error("invalid contact settings: {reason}")]
    InvalidContactSettings { reason: String },
    #[error("system settings persistence error: {reason}")]
    Persistence { reason: String },
}

#[derive(Error, Debug, Clone)]
pub enum UserAdminError {
    #[error("invalid role: '{role}' (allowed: admin, operator, viewer)")]
...
pub enum NotificationError {
    #[error("notifier transport failed: {reason}")]
