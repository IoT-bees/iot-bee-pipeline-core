use domain::value_objects::lifecycle_values::ReplicaFlowTelemetry;
...
// ─── Telemetría del flujo ───────────────────────────────────────────────────

#[test]
fn telemetria_del_flujo_combina_las_etapas_de_los_tres_actores() {
    let mut consumer = ReplicaFlowTelemetry::default();
    consumer.record_received();
    consumer.record_received();

    let mut processor = ReplicaFlowTelemetry::default();
    processor.record_validated();
    processor.record_rejected();

    let mut store = ReplicaFlowTelemetry::default();
    store.record_delivered();

    let telemetry = ReplicaFlowTelemetry::merge(consumer, processor, store);

    assert_eq!(telemetry.received().count(), 2);
    assert_eq!(telemetry.validated().count(), 1);
    assert_eq!(telemetry.rejected().count(), 1);
    assert_eq!(telemetry.delivered().count(), 1);
    assert!(telemetry.received().last_activity_at().is_some());
}
