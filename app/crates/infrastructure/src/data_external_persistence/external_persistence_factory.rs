...
use super::webhook_persistence::WebhookPersistence;
...
impl ExternalPersistenceFactory {
    pub fn create_external_persistence(
        ...
        match config {
            ...
            PipelineDataStoreModel::LocalLog(_) => Err(
                "Local file storage is not available yet. Please choose InfluxDB as the destination."
                    .into(),
            ),
            PipelineDataStoreModel::Webhook(webhook_config) => {
                Ok(Arc::new(WebhookPersistence::new(webhook_config)?))
            }
        }
    }
}
