...
impl PipelineGroupRepository for GroupRepository {
    ...
    async fn save_pipeline_group(
        ...
        let result = sqlx::query(
            r#"
            INSERT INTO pipeline_groups (name, description, created_at, updated_at, organization_id) VALUES ($1, $2, $3, $4, $5)
            "#,
        )
        .bind(&group.name())
    ...
    async fn get_pipelines_using_group(
        ...
        let rows = sqlx::query(
            r#"
            SELECT id FROM pipelines WHERE group_id = $1 AND organization_id = $2
            "#,
        )
        .bind(pipeline_id_to_database(group_id.id()))
        .bind(org_id)
    ...
    async fn delete_pipeline_group(
        ...
        let result = sqlx::query(
            r#"
            DELETE FROM pipeline_groups WHERE id = $1 AND organization_id = $2
            "#,
        )
        .bind(pipeline_id_to_database(group_id.id()))
        .bind(org_id)
