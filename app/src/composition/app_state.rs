...
use infrastructure::persistence::repositories::license_repository::PostgresLicenseRepository;
...
use infrastructure::persistence::repositories::users_repository::PostgresUserRepository;
...
use application::usage_cases::cases::{UsageUseCases, UsageUseCasesImpl};
...
use infrastructure::persistence::repositories::audit_events_repository::PostgresAuditEventsRepository;
use infrastructure::persistence::repositories::contact_settings_repository::PostgresContactSettingsRepository;
use infrastructure::persistence::repositories::organizations_repository::PostgresOrganizationsRepository;
use infrastructure::persistence::repositories::plans_repository::PostgresPlansRepository;
use infrastructure::persistence::repositories::usage_repository::PostgresUsageRepository;
...
impl AppState {
    pub fn new(internal_data_base: Arc<InternalDataBase>) -> Self {
        ...
        let audit_repo_for_cron: Arc<dyn AuditRepository> = Arc::new(
            PostgresAuditEventsRepository::new(internal_data_base.clone()),
        );
    ...
    pub async fn build_db() -> Result<Arc<InternalDataBase>, Box<dyn std::error::Error>> {
        ...
        sqlx::migrate!("./migrations_postgres")
            .run(db.pool())
            .await?;
        PostgresUsageRepository::new(db.clone())
            .clear_reservations_after_restart()
            .await?;
        Ok(db)
    }
    ...
    pub fn pipeline_data_app_state(&self) -> web::Data<dyn PipelineDataUseCases + Send + Sync> {
        ...
        let license_repo: Arc<PostgresLicenseRepository> = Arc::new(
            PostgresLicenseRepository::new(self.internal_data_base.clone()),
        );
    ...
    pub fn license_app_state(&self) -> web::Data<dyn LicenseUseCases + Send + Sync> {
        ...
        let users_repo: Arc<dyn UserRepository> =
            Arc::new(PostgresUserRepository::new(self.internal_data_base.clone()));
    ...
    fn usage_use_case(&self) -> Arc<UsageUseCasesImpl<PostgresLicenseRepository>> {
        let usage_repo: Arc<dyn UsageRepository> = Arc::new(PostgresUsageRepository::new(
            self.internal_data_base.clone(),
        ));
        let licenses = Arc::new(PostgresLicenseRepository::new(
            self.internal_data_base.clone(),
        ));
        let users: Arc<dyn UserRepository> =
            Arc::new(PostgresUserRepository::new(self.internal_data_base.clone()));
        let audit: Arc<dyn AuditRepository> = Arc::new(PostgresAuditEventsRepository::new(
            self.internal_data_base.clone(),
        ));
        Arc::new(UsageUseCasesImpl::new(
            usage_repo,
            licenses,
            self.plan_repo(),
            users,
    ...
    pub fn usage_app_state(&self) -> web::Data<dyn UsageUseCases + Send + Sync> {
        let usage: Arc<dyn UsageUseCases + Send + Sync> = self.usage_use_case();
        web::Data::from(usage)
    }
    ...
    pub fn pipeline_lifecycle_app_state(
        ...
        let license_repository = Box::new(PostgresLicenseRepository::new(
            self.internal_data_base.clone(),
        ));
    ...
    pub async fn ensure_default_admin(&self) {
        let repo = PostgresUserRepository::new(self.internal_data_base.clone());
    ...
    pub fn auth_app_state(&self) -> web::Data<dyn AuthUseCases + Send + Sync> {
        let repo = Arc::new(PostgresUserRepository::new(self.internal_data_base.clone()));
    ...
    pub fn audit_repo(&self) -> Arc<dyn AuditRepository> {
        Arc::new(PostgresAuditEventsRepository::new(
            self.internal_data_base.clone(),
        ))
    }
    ...
    pub fn system_app_state(&self) -> web::Data<dyn SystemUseCases + Send + Sync> {
        ...
        let contact_settings = Arc::new(PostgresContactSettingsRepository::new(
            self.internal_data_base.clone(),
        ));
        let uc: Arc<dyn SystemUseCases + Send + Sync> =
            Arc::new(SystemUseCasesImpl::new(probe, contact_settings));
        web::Data::from(uc)
    }
    ...
    pub fn organization_app_state(&self) -> web::Data<dyn OrganizationUseCases + Send + Sync> {
        let repo = Arc::new(PostgresOrganizationsRepository::new(
            self.internal_data_base.clone(),
        ));
    ...
    pub fn plan_repo(&self) -> Arc<dyn PlanRepository> {
        Arc::new(PostgresPlansRepository::new(
            self.internal_data_base.clone(),
        ))
    }
