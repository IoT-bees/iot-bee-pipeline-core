...
/// Returns `None` if the field is missing or malformed; callers fall back to
/// the legacy single-tenant default.
...
pub async fn sync_stripe_billing(
    ...
    let client_ref = raw_payload
        ...
        .and_then(|v| v.as_str());
    let org_id = parse_client_reference_org(client_ref).unwrap_or_else(|| {
        // TODO(plan-07): once Stripe Checkout always emits `client_reference_id`
        // = `org:<id>`, drop this fallback and 400 on missing org context.
        LOGGER.warn("stripe-sync missing/invalid clientReferenceId; falling back to default org 1");
        1
    });
