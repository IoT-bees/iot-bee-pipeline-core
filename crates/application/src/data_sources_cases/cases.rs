    ...
    T: PipelineDataSourceRepository + Send + Sync,
{
    async fn create_data_source(
        ...
        self.repository
            .save_pipeline_data_source(org_id, data_source)
            .await
            .map_err(|e| {
    ...
    async fn get_data_source(
        ...
        let result = self
            .repository
            .get_pipeline_data_source(org_id, &data_source_id)
            .await
            .map_err(|e| {
                LOGGER.error(&format!(
    ...
    async fn list_data_sources(
        ...
        let result = self
            .repository
            .list_pipeline_data_source(org_id)
            .await
            .map_err(|e| {
    ...
    async fn test_data_source(
        ...
        let data_source = self.get_data_source(org_id, data_source_id).await?;
        Ok(format!(
            "Saved {} configuration loaded. This is not a live connection test — the broker is contacted when the pipeline starts.",
    ...
    async fn update_data_source(
        ...
        let update_result = self
            .repository
            .update_pipeline_data_source(org_id, &data_source_id, data)
            .await
            .map_err(|e| {
                LOGGER.error(&format!(
    ...
    async fn update_data_source_name(
        ...
        self.repository
            .update_pipeline_data_source_name(org_id, &data_source_id, &field_name)
            .await
            .map_err(|e| {
                LOGGER.error(&format!(
    ...
    async fn delete_data_source(
        ...
        let pipelines_using_data_source = self
            .repository
            .get_pipelines_using_data_source(org_id, &data_source_id)
            .await
            .map_err(|e| {
                LOGGER.error(&format!(
        ...
        if !pipelines_using_data_source.is_empty() {
            let count = pipelines_using_data_source.len();
            LOGGER.warn(&format!(
            ...
            return Err(PipelinePersistenceError::DeleteFailed {
                reason: format!(
                    "This data source is used by {count} {pipelines_word}. Reassign or delete those {pipelines_word} first, then try again."
                ),
            }
            .into());
        }
        ...
        self.repository
            .delete_pipeline_data_source(org_id, &data_source_id)
            .await
            .map_err(|e| {
                LOGGER.error(&format!(
