...
pub trait DataStoreUseCases {
    ...
    async fn delete_data_store(&self, org_id: i64, data_store_id: &u32) -> Result<(), IoTBeeError>;
}
    ...
    T: PipelineDataStoreRepository + Send + Sync,
{
    async fn create_data_store(
        ...
        self.repository
            .save_pipeline_data_store(org_id, data_store)
            .await
    }
    async fn get_data_store(
        &self,
        ...
        self.repository.get_pipeline_data_store(org_id).await
    }
    async fn get_data_store_by_id(
        &self,
        org_id: i64,
        ...
        let result = self
            .repository
            .get_pipeline_data_store_by_id(org_id, &data_store_id)
            .await?;
    ...
    async fn test_data_store(
        ...
        let data_store = self.get_data_store_by_id(org_id, data_store_id).await?;
        Ok(format!(
            "Saved {} configuration loaded. This is not a live connection test — the destination is contacted when the pipeline writes its first record.",
    ...
    async fn update_data_store_configuration(
        ...
        self.repository
            .update_pipeline_data_store_configuration(org_id, &data_store_id, new_config)
            .await
    }
    async fn delete_data_store(&self, org_id: i64, data_store_id: &u32) -> Result<(), IoTBeeError> {
        let data_store_id = DataStoreId::new(*data_store_id)?;
        ...
        self.repository
            .delete_pipeline_data_store(org_id, &data_store_id)
            .await
    }
}
