...
impl LicenseRepository for StubLicenseRepo {
    ...
    async fn upsert_subscription(&self, _: &LicenseSubscription) -> Result<(), IoTBeeError> {
        Ok(())
    }
    ...
    async fn list_billing_events(&self, _: i64) -> Result<Vec<BillingEvent>, IoTBeeError> {
        Ok(vec![])
    }
    async fn get_billing_event(&self, _: i64) -> Result<Option<BillingEvent>, IoTBeeError> {
        Ok(None)
    }
