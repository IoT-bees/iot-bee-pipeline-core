...
impl LicenseRepository for SqliteLicenseRepository {
    ...
    async fn list_billing_events(&self, limit: i64) -> Result<Vec<BillingEvent>, IoTBeeError> {
        ...
        let rows: Vec<(i64, String, String, String, String, i64, Option<String>)> = sqlx::query_as(
            "SELECT id, stripe_event_id, event_type, payload, created_at, processed_ok, last_error \
                 FROM billing_events ORDER BY id DESC LIMIT ?",
        )
        .bind(limit)
        .fetch_all(self.pool())
        .await
        ...
        Ok(rows
            .into_iter()
            .map(|(id, ev, ty, pl, ca, ok, err)| BillingEvent {
                id,
                stripe_event_id: ev,
                event_type: ty,
                payload: pl,
    ...
    async fn get_billing_event(&self, id: i64) -> Result<Option<BillingEvent>, IoTBeeError> {
        let row: Option<(i64, String, String, String, String, i64, Option<String>)> =
            sqlx::query_as(
                "SELECT id, stripe_event_id, event_type, payload, created_at, processed_ok, last_error \
        ...
        Ok(row.map(|(id, ev, ty, pl, ca, ok, err)| BillingEvent {
            id,
            stripe_event_id: ev,
            event_type: ty,
            payload: pl,
    ...
    async fn mark_billing_event_processed(
        ...
        sqlx::query("UPDATE billing_events SET processed_ok = ?, last_error = ? WHERE id = ?")
