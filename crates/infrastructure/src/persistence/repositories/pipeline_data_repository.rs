...
impl PipelineControllerRepository for PipelineDataRepository {
    ...
    async fn count_pipelines(&self, org_id: i64) -> Result<u32, IoTBeeError> {
        let pool = self.data_base_connection().pool();
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM pipelines WHERE organization_id = ?")
                .bind(org_id)
                .fetch_one(pool)
                .await
                .map_err(|e| {
    ...
    async fn get_pipeline_by_group_id(
        ...
        let rows_result = sqlx::query_as::<_, PipelineRowFlat>(
            ...
            JOIN data_sources ds ON p.data_source_id = ds.id
            JOIN validation_schemas vs ON p.validation_schema_id = vs.id
            WHERE pg.id = ? AND p.organization_id = ?
            "#,
        )
        .bind(group_id.id())
        .bind(org_id)
    ...
    async fn update_pipeline_data_source(
        ...
        let result = sqlx::query(
            ...
            WHERE id = ? AND organization_id = ?
            AND EXISTS (
                SELECT 1 FROM data_sources WHERE id = ? AND organization_id = ?
            )
            "#,
        )
        ...
        if result.rows_affected() == 0 {
            ...
            let pipeline_exists: bool = sqlx::query_scalar(
                "SELECT EXISTS(SELECT 1 FROM pipelines WHERE id = ? AND organization_id = ?)",
            )
            .bind(pipeline_id.id())
            .bind(org_id)
            .fetch_one(pool)
    ...
    async fn update_pipeline_data_store(
        ...
        let result = sqlx::query(
            ...
            WHERE id = ? AND organization_id = ?
            AND EXISTS (
                SELECT 1 FROM databases WHERE id = ? AND organization_id = ?
            )
            "#,
        )
    ...
    async fn update_pipeline_validation_schema(
        ...
        let result = sqlx::query(
            ...
            WHERE id = ? AND organization_id = ?
            AND EXISTS (
                SELECT 1 FROM validation_schemas WHERE id = ? AND organization_id = ?
            )
            "#,
        )
    ...
    async fn update_pipeline_group(
        ...
        let result = sqlx::query(
            ...
            WHERE id = ? AND organization_id = ?
            AND EXISTS (
                SELECT 1 FROM pipeline_groups WHERE id = ? AND organization_id = ?
            )
            "#,
        )
        .bind(group_id.id())
