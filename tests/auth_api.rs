...
fn auth_data(db: Arc<InternalDataBase>) -> web::Data<dyn AuthUseCases + Send + Sync> {
    ...
    let notifications: Arc<dyn NotificationsUseCases> =
        Arc::new(NotificationsUseCasesImpl::new(Arc::new(LogNotifier)));
    let uc: Arc<dyn AuthUseCases + Send + Sync> =
        Arc::new(AuthUseCasesImpl::new(repo, hasher, issuer, notifications));
    web::Data::from(uc)
}
...
fn audit_repo(db: Arc<InternalDataBase>) -> Arc<dyn AuditRepository> {
    Arc::new(SqliteAuditEventsRepository::new(db))
}
...
async fn first_register_then_me_works() {
    ...
    let app = test::init_service(
        App::new()
            .service(auth_scope(auth.clone(), audit_repo(db.clone())))
            .service(
                web::scope("")
                    .app_data(auth.clone())
                    .wrap(JwtAuth)
...
async fn second_register_returns_403() {
    ...
    let auth = auth_data(db.clone());
    let app =
        test::init_service(App::new().service(auth_scope(auth.clone(), audit_repo(db.clone()))))
            .await;
