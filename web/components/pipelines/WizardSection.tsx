...
export function WizardSection({
  ...
  const statusColor =
    status === "ready"
      ...
      : "text-[var(--color-fg-4)]";
  return (
    <div className="border border-[#1f1f1f] mb-3 bg-[var(--color-bg-panel)]">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
      ...
      >
        ...
        <span className="text-[12px] text-[var(--color-fg-3)] tracking-[2px]">
          STEP {step}
        </span>
      ...
      {open && (
        <div className="px-4 py-4 border-t border-[#1f1f1f]">{children}</div>
      )}
    </div>
  );
}
