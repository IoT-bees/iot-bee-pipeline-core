...
import { rawDataSourceSchema, validated } from "../contracts";
...
export const sourcesApi = {
  list: async (): Promise<DataSource[]> => {
    const raw = await api<RawDataSource[]>(
      "/data-sources",
      {},
      validated(rawDataSourceSchema.array()),
    );
  ...
  update: (id: number, b: CreateDataSourceRequest): Promise<void> =>
    api<null>(`/data-sources/${id}`, {
      method: "PUT",
      body: JSON.stringify(b),
    }).then(() => undefined),
  remove: (id: number) =>
