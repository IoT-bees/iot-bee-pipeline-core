...
export function useSchemas() {
  return useQuery({ queryKey: ["schemas"], queryFn: schemasApi.list, staleTime: 30_000 });
}
...
export function useCreateSchema() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      push({ kind: "success", message: "Esquema creado." });
    },
...
export function useDeleteSchema() {
  ...
  const push = useToasts((s) => s.push);
  return useMutation({
    mutationFn: (id: number) => idempotentDelete(schemasApi.remove(id)),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["schemas"] });
      push({ kind: "success", message: "Esquema eliminado." });
    },
    onError: (e: Error) => push({ kind: "error", message: e.message }),
  });
}
